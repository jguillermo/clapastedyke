...
| Documento | Qué cubre |
|---|---|
| [`deploy/google-client-id.md`](../deploy/google-client-id.md) | **Los pasos para crear el Client ID y el client secret de Google** (proyecto de Cloud, consentimiento, orígenes autorizados) y dónde acaba cada uno: el ID en `deploy/firebase/environments.json` —de donde se derivan `config.json` y el `.env` de la función— y el secreto en `deploy/.env-secret` y el *environment* de GitHub. Es **solo OAuth**: la infraestructura del ambiente está en [`api.md`](api.md). El **único** sitio con ese procedimiento: no se duplica aquí. |
