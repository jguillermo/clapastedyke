...
| `"rewrites"`: `/api/auth/**` → función `auth` | El backend de la sesión, servido como **mismo origen** que la app. Es lo que permite que la cookie `HttpOnly; SameSite=Lax` viaje — ver [`functions.md`](functions.md). Es el **único** rewrite: el `"source": "**"` que mete `firebase init` se quitó, y si vuelve a aparecer se tragaría `/api/auth/token` (no lleva punto) |
