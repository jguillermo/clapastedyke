...
import { ShoppingListItem } from '../../business/inventory/application/shopping-list/shopping-list';
import { PurchaseListItem } from '../../business/inventory/application/list-purchases/list-purchases';
...
export class PurchasesScreen {
  ...
  private async loadCatalogs(): Promise<void> {
    ...
    this.orders.set(await this.business.listOrders.execute({}));
  }
  ...
  protected async loadItems(): Promise<void> {
    ...
    if (this.mode() === 'manual') {
      list = await this.business.suppliesBelowMinimum.execute();
    } else if (this.orderId()) {
      list = await this.business.orderShortages.execute({ orderId: this.orderId() });
    }
    list = [...list].sort((a, b) =>
  ...
  protected save(): void {
    void submit(this.form, async () => {
      ...
      try {
        const m = this.model();
        const r = await this.business.registerPurchase.execute({
  ...
  protected async reloadHistory(): Promise<void> {
    ...
    this.purchases.set(await this.business.listPurchases.execute());
