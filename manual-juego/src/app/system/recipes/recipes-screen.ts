...
import { BaseType, RecipePrimitives } from '../../business/catalog/domain/recipe/recipe';
...
export class RecipesScreen {
  ...
  private async loadSupplies(): Promise<void> {
    this.supplies.set(await this.business.listSupplies.execute({ type: 'ingredient' }));
  }
  ...
  protected save(): void {
    void submit(this.form, async () => {
      ...
      this.notice.set(null);
      try {
        const r = await this.business.saveRecipe.execute({
