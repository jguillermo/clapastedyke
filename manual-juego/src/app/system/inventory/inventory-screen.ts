...
import { StockLight } from '../../business/catalog/domain/supply/supply';
import { SupplyListItem } from '../../business/catalog/application/list-supplies/list-supplies';
import { AdjustmentType } from '../../business/settings/domain/business-settings';
import { StockMovementListItem } from '../../business/inventory/application/list-stock-movements/list-stock-movements';
...
export class InventoryScreen {
  ...
  private async load(): Promise<void> {
    ...
    const [supplies, settings] = await Promise.all([
      this.business.listSupplies.execute({}),
  ...
  protected async reloadKardex(): Promise<void> {
    ...
    this.movements.set(
      await this.business.listStockMovements.execute(filter ? { supplyId: filter } : {}),
    );
  }
  ...
  protected async updatePreview(): Promise<void> {
    ...
    try {
      const r = await this.business.previewAdjustment.execute({
        supplyId: m.supplyId,
        type: m.type,
        quantity: m.quantity,
      });
  ...
  protected save(): void {
    void submit(this.form, async () => {
      ...
      try {
        const m = this.model();
        const r = await this.business.adjustInventory.execute({
          supplyId: m.supplyId,
          type: m.type,
          quantity: m.quantity,
