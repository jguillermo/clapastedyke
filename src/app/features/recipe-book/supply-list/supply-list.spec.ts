...
import { makeSupply, makeRecipeBookFakes } from '@core/recipe-book/testing/recipe-book-test-doubles';
import { SupplyRepository } from '@core/recipe-book/domain/repositories/supply.repository';
...
describe('SupplyList', () => {
  ...
  async function render(
    seed: ReturnType<typeof makeSupply>[] = [],
  ): Promise<{ fixture: ComponentFixture<SupplyList>; internals: ListInternals }> {
    TestBed.configureTestingModule({
      imports: [SupplyList],
      providers: makeRecipeBookFakes().providers,
    });
    repo = TestBed.inject(SupplyRepository);
    for (const supply of seed) {
      await repo.save(supply);
    }
    const fixture = TestBed.createComponent(SupplyList);
    fixture.componentRef.setInput('supplies', seed);
  ...
  it('seeds one editable row per existing supply plus a trailing blank row', async () => {
    const { internals } = await render([makeSupply('IN-1', 'Harina', { amount: 5, per: Quantity.of(1000, 'g') })]);
  ...
  it('writes a new supply from the trailing row (create) and adds a fresh blank row', async () => {
    const { internals } = await render([]);
  ...
  it('edits the price of an existing supply in place (no duplicate)', async () => {
    const { internals } = await render([makeSupply('IN-1', 'Harina', { amount: 5, per: Quantity.of(1000, 'g') })]);
  ...
  it('renames an existing supply in place keeping its identity', async () => {
    const { internals } = await render([makeSupply('IN-1', 'Harina', { amount: 5, per: Quantity.of(1000, 'g') })]);
  ...
  it('shows an error and keeps the original name when renaming collides with another insumo', async () => {
    const { internals } = await render([
      ...
      makeSupply('IN-2', 'Azúcar', { amount: 4, per: Quantity.of(1000, 'g') }),
    ]);
