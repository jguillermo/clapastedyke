...
describe('SpongeRecipeMapper', () => {
    it('round-trips a sponge recipe with yield (weight, servings, size), flavor and lines', () => {
        const original = SpongeRecipe.create(
            new EntityId('SP-1'),
            'Queque de vainilla',
            RecipeYield.of(Quantity.of(1000, 'g'), 8, 'Mediano'),
            [
        ...
        expect(restored.referenceYield.size).toBe('Mediano');
