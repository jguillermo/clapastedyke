...
describe('MeasureInput', () => {
  ...
  it('is equal by normalised value and unit', () => {
    expect(MeasureInput.parse('1 kg', 'mass').equals(MeasureInput.parse('1000 g', 'mass'))).toBe(
      false,
    );
    // misma unidad de visualización + mismos gramos → iguales
    expect(MeasureInput.parse('1kg', 'mass').equals(MeasureInput.parse('1 kilo', 'mass'))).toBe(
      true,
    );
  });
});
